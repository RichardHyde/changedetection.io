...
class ITunesSettingsForm(Form):
    ...
    hd_price = BooleanField('HD Price', default=False)

class processor_settings_form(processor_text_json_diff_form):
    itunes_settings = FormField(ITunesSettingsForm)
        ...
        <fieldset id="restock-fieldset-price-group">
            <div class="pure-control-group">
                <fieldset class="pure-group inline-radio">
                    {{ render_field(form.itunes_settings.in_stock_processing) }}
                </fieldset>
                <fieldset class="pure-group">
                    {{ render_checkbox_field(form.itunes_settings.follow_price_changes) }}
                    <span class="pure-form-message-inline">Changes in price should trigger a notification</span>
                </fieldset>
                <fieldset class="pure-group">
                    {{ render_checkbox_field(form.itunes_settings.lowest_price_only) }}
                    <span class="pure-form-message-inline">Trigger a notification only when the lowest price is reached</span>
                </fieldset>
                <fieldset class="pure-group price-change-minmax">               
                    {{ render_field(form.itunes_settings.price_change_min, placeholder=watch.get('restock', {}).get('price')) }}
                ...
                <fieldset class="pure-group price-change-minmax">
                    {{ render_field(form.itunes_settings.price_change_max, placeholder=watch.get('restock', {}).get('price')) }}
                ...
                <fieldset class="pure-group price-change-minmax">
                    {{ render_field(form.itunes_settings.price_change_threshold_percent) }}
                ...
                <fieldset>
                    {{ render_checkbox_field(form.itunes_settings.hd_price) }}
                    <span class="pure-form-message-inline">Use the HD price if available/applicable.</span><br>
                </fieldset>              
            </div>
        </fieldset>
        """
